...
- Every scripted friend-message (opener included) presents with the same perceived cadence as Build's real AI streaming: brief `TypingBubble` pause, then the line reveals progressively rather than snapping in as a block. Since the lines are hard-coded (no real token stream to drive it), the reveal is a synthetic word-by-word timer, `SCRIPTED_REVEAL_MS_PER_WORD` (default 40ms/word) in `OnboardingChat`, tuned to approximate Build's progressive-arrival feel. Purely presentational — never affects when profile fields are written or when the handoff fires.

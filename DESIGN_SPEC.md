...
- The reactive reflection line shown after each answer (replacing the old flat "Got it.") renders through the exact same `ChatBubble`/reveal path as every other scripted AI line — no visual distinction, no separate styling. The only difference is timing-internal, not visual: its typing-indicator dots run for the real bounded network wait rather than the fixed `SCRIPTED_TYPING_INDICATOR_MS` pause used elsewhere, so it can look slightly shorter or longer dots-duration than a plain scripted line, never a different appearance. See FEATURE_SPECS.md's "Onboarding — Conversational Flow" section for the reflection/parser mechanics and the pacing rationale.
- Closing handoff line, after the constraints reflection: a single scripted line, "Perfect — that's everything I need. Setting up your kitchen around this now." — same `ChatBubble`/`sayAI()` presentation as any other line, no profile recap (the per-answer reflections already covered that). The slide transition into the "Setting up your kitchen…" loading screen (see the Handoff screen bullet above) does not begin until this line has fully revealed, so it can never be visually cut off mid-sentence — a fix for an on-phone-observed bug where an earlier two-line closing beat (a full recap line + a "Give me a second…" line) got cut short by the transition. Purely a copy/ordering fix — the loading screen's own poll timing is unaffected.

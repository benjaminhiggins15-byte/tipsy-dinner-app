...
Keep it achievable. They are cooking this themselves, at home. Favor a couple of high-leverage moves that lift the result — a technique explained plainly, a finishing touch, a smart shortcut — over a single difficult or fussy step. Excellent food a home cook can actually pull off, with the method demystified the way a friend showing you would, not a chef performing.
The goal is that they end up genuinely proud of what they made — the sense of accomplishment that comes from putting something really good in front of people.
Your two modes:
...
Before giving suggestions, make sure you know enough to make them genuinely good. When the ask is broad, the thing to establish first is the type of food they're in the mood for — the cuisine or culinary world. That's the question a friend asks first, because it narrows everything downstream: the ingredients, the techniques, the whole universe of ideas. Ask for it directly and naturally, the way a friend looking for direction would.
A named protein does not settle this. If someone says "something with chicken" or "a pork dish" but the culinary direction is still open, that's exactly the moment to ask which world they're leaning toward — because a protein alone spans every cuisine, and guessing produces a scattered, random-feeling set. The exception is when their ask already implies a direction, or their palate profile clearly points one way — then don't ask, just go.
Once there is a direction — whether they gave it or you asked — let it anchor the whole set of suggestions so they cohere. Five ideas within one world that rhyme with each other, not five unrelated cuisines stapled to the same ingredient. Save execution details (sheet pan vs. skillet, make-ahead vs. cook-in-the-moment) for after there's a direction; those you can decide for them the way a friend would.
Usually one question is enough, and it's fine to fold two closely related things into a single natural question. Occasionally ask a second turn's worth when the first answer opens a genuinely important unknown — but stop there, and never make it feel like an interview. If the request is already specific enough to give great ideas, skip the questions and go straight to suggestions.
...
Don't describe your own recipes in superlatives or absolutes — no "perfect," "best," "ultimate," "the definitive." Name the dish plainly and hand it over with confidence; let the food speak for itself. This isn't a reason to hedge or undersell either — "a solid attempt at" is as wrong as "the perfect." Confident and plain, not boastful, not modest.
Never pepper the user with questions or make it feel like an interview. Ask only what genuinely improves what you can offer, then commit to something useful.
...
function renderScreen(
  ...
  switch (s.name) {
    ...
    case "newcategoryforreceived": return (
      <NewCategory
        back={back}
        onSaved={(cat) => {
...
export default function App() {
  ...
  const ensureRecipesLoaded = async (categoryKey: string, categoryLabel: string) => {
    ...
    const recipes = categoryKey === "__all__"
      ? await getSavedRecipesAll()
      : await getRecipesForCategory(categoryKey, categoryLabel);
  ...
  const clearRecipeCache = (categoryKey: string) => {
    setRecipesByCategory(prev => {
      ...
      // "__all__" aggregates every category, so it's stale after any single-category
      // mutation too — clear it here so no call site has to remember a second key.
      delete next["__all__"];
      return next;
    });
  };
  ...
  // Mirrors finishCreateCategoryForRecipe's "cook" branch — receiving a
  // recipe has no addown-equivalent, so there's only one return target.
  const finishCreateCategoryForReceived = (catKey: string, catLabel: string, item: PendingReceivedRecipe) => {
    if (transition) return;
    updateCurrentTabStack((prev) => {
      const newStack = prev.slice(0, -1); // Remove newcategoryforreceived screen
      return [...newStack, { name: "receivedRecipe", item, newCategory: { key: catKey, label: catLabel } }];
    });
  };
  ...
  const finishSaveRecipe = (recipe: Recipe, categoryKey: string, categoryLabel: string) => {
    ...
    setTabStacks((stacks) => ({
      ...
      home: [{ name: "home" }], // Reset Home tab to root — prevents a just-saved received recipe from lingering as a stale receivedRecipe/receivedPending screen
      recipes: [
        { name: "categories" },
